...
if ($result->num_rows > 0) {
    ...
    // Validate password
    if ((isset($teacher['t_password']) && password_verify($password, $teacher['t_password'])) || $password === $teacher['t_password']) {

        // Check if teacher is a dean in the database
        ...
        $_SESSION['is_dean']    = $is_dean; // true if teacher is also a dean

        // Always redirect to teacher dashboard by default
        echo json_encode([
            'success'  => true,
    ...
    echo json_encode(['error' => 'Teacher account not found']);
    exit();
}
