                                ...
                                <?php if ($result && $result->num_rows > 0): ?>
                                    <?php while ($row = $result->fetch_assoc()): ?>
                                       ...
                                       $full_name = htmlspecialchars(
                                            $row['p_lname'] . 
                                            (!empty($row['p_suffix']) ? ' ' . $row['p_suffix'] : '') . // suffix right after last name
                                            ', ' . $row['p_fname'] .
                                            (!empty($row['p_mname']) ? ' ' . strtoupper(substr($row['p_mname'], 0, 1)) . '.' : '')
                                        );

                                        // Child display
                                        $child_name = $row['s_fname'] ? "{$row['s_fname']} {$row['s_lname']}" : 'No child linked';
                                        $child_section = $row['section_code'] ?
                                            $row['section_code'] : '<span class="badge bg-warning text-dark">Not yet Assigned</span>';
                                        ...
                                        ?>
                                        <tr 
                                            data-parent-id="<?= htmlspecialchars($row['p_id']) ?>"
                                            ...
                                            data-parent-bdate="<?= date('Y-m-d', strtotime($row['p_bdate'])) ?>"
                                        ...
                                        >
                                            ...
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <?php elseif (!empty($row['profile_blob'])): ?>
                                                    <!-- ✅ Blob-based avatar -->
                                                    <img src="data:image/jpeg;base64,<?= base64_encode($row['profile_blob']) ?>" 
                                            ...
                                            <td><?= $child_display ?></td>
                                            <td>
                                                <span class="badge bg-<?= $row['p_status'] === 'active' ? 'success' : 'danger' ?>">
                                                    <?= ucfirst($row['p_status']) ?>
                                                </span>
                                            </td>
                                            <td><?= htmlspecialchars($row['p_email']) ?></td>
                                            <td><?= htmlspecialchars($row['p_gender']) ?></td>
                                            <td><?= date('Y-m-d', strtotime($row['p_bdate'])) ?></td>
...
$(document).ready(function() {
   window.parentsTable = $('#parentsTable').DataTable({
    ...
    scrollCollapse: true,
    paging: true,
    responsive: true,          // fully responsive
    autoWidth: false,          // prevent forcing widths
    columnDefs: [
        { className: 'dtr-control', orderable: false, targets: 0 },
        { targets: [5,6,7,8,9], visible: false } // hide extra columns
    ],
    order: [[1, 'asc']]
});
    ...
    $('#parentsTable tbody').on('click', 'tr', function() {
        ...
        $('#parentDetailsBody').html(`
            ...
            <div class="action-buttons d-flex justify-content-center gap-2 mt-3">
                <button class="btn btn-sm btn-primary btn-edit-parent" data-bs-toggle="modal" data-bs-target="#editParentModal" data-parent-id="${parentId}">
                    <i class="bi bi-pencil-square me-1"></i>Edit
                </button>
                <button class="btn btn-sm btn-danger btn-delete-parent" data-parent-id="${parentId}">
