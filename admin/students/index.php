...
$base_url = '/admin/students/processes';

// =======================
// Fetch ALL students in active term
// =======================
$sql = "
SELECT 
    s.*,
...
LEFT JOIN (
    ...
    ON s.s_id = sec_single.s_id AND s.is_regular = 1
...
LEFT JOIN (
    ...
    WHERE se.is_status = 1
      AND se.enrollment_status = 'Enrolled'
    GROUP BY se.s_id
) AS sec_merged
    ON s.s_id = sec_merged.s_id AND s.is_regular != 1
...
<script src="/admin/students/js/add_student.js"></script>
<script src="/admin/students/js/edit_student.js"></script>
<script src="/admin/students/js/delete_student.js"></script>
...
studentsTable.on('draw', function () {
    ...
    document.getElementById('exportStudentsBtn').addEventListener('click', function() {
        fetch('/admin/students/processes/export_unassigned.php')
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                    showAlert(data.message || 'Export successful.', 'success');
                    const link = document.createElement('a');
                    link.href = '/admin/students/processes/download_temp.php?file=' + encodeURIComponent(data.file);
