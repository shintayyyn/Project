...
try {
    ...
    // -----------------------------
// 10. Auto Generate QR Code for All Students
// -----------------------------
$full_name = trim("{$s_fname}" . ($s_mname ? " {$s_mname}" : "") . " {$s_lname}" . ($s_suffix ? " {$s_suffix}" : ""));
$section_code = $_POST['section'] ?? 'N/A'; // you can change this later when assigning section
...
if ($checkQR->num_rows === 0) {
    ...
    $insertQR = $conn->prepare("
        ...
        VALUES (?, ?, ?, ?, NOW(), NOW())
    ");
    $insertQR->bind_param("isss", $s_id, $full_name, $section_code, $qrCodeText);
    $insertQR->execute();
    $insertQR->close();
} else {
    $checkQR->close();
   ...
   echo json_encode([
    'success' => true,
    'message' => $is_solo === 2 ? 'Student added successfully as Solo with QR code generated.' :
                (!empty($_POST['existing_parent_id']) ? 'Student linked to existing parent with QR code generated.' : 'Student and parent added successfully with QR code generated.'),
    'data' => [
        's_id' => $s_id,
        'p_id' => $p_id,
        'idcode' => $student_idcode,
        'generated_qrcode' => $qrCodeText, // ✅ added
        's_fname' => $s_fname,
        's_mname' => $s_mname,
        's_lname' => $s_lname,
        's_suffix' => $s_suffix,
