...
while ($student = $students_result->fetch_assoc()) {
    // If the student doesn't have a degree_code, fallback to 'UNKNOWN'
...
// =========================
// 4. Fetch Sections by Term for Legend
// =========================
$sections_by_term = [];
...
while ($term = $terms_res->fetch_assoc()) {
    ...
    $sec_stmt = $conn->prepare("
        ...
        WHERE sec.term_id = ?
        ORDER BY d.degree_code ASC, sec.section_code ASC
    ");
    $sec_stmt->bind_param("i", $term['term_id']);
    $sec_stmt->execute();
...
// =========================
// 7. Save Spreadsheet and Return JSON
// =========================
