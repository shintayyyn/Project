                        ...
                        <div class="col-12 mx-auto mb-4">
                            <div class="card shadow-sm h-100 d-flex flex-column text-center text-sm-start"> <!-- Header -->
                                ...
                                <button
                                    ...
                                    <h4><i class="bi bi-qr-code"></i> Log Attendance</h4>
                                </button>
                        ...
                        <table id="attendanceTable" class="table table-bordered table-hover">
                            ...
                            <tbody>
                                <?php foreach ($attendance_history as $att): ?>
                                    ...
                                    $status = $att['status'];
                                    $badgeClass =
                                        $status === 'Present' ? 'bg-success text-white' : ($status === 'Absent' ? 'bg-danger text-white' : ($status === 'Late' ? 'bg-warning text-dark' : ($status === 'Excuse' ? 'bg-primary text-white' : 'bg-secondary text-white')));
                                    ?>
                                    <tr>
            ...
            `;
                ...
                try {
                    const response = await fetch('generate_temp_qr.php', {
                        ...
                        body: 'ss_id=' + encodeURIComponent(ssId) +
                    ...
                    if (data.status === 'success') {
                        ...
                        const formattedTime = expires.toLocaleTimeString([], {
                            hour: '2-digit',
                            minute: '2-digit'
                        });
